var mainChart;
var url = "ws://" + window.location.host + "/devHistory";
...
$(document).ready(function() {
	...
	$('#devices-selectpicke').selectpicker();
	...
	$("#btn-create-charts").click(function() {
		...
		var sendMsg = ids + "," + startDateStr + "," + endDateStr;
		mainChart.clear();
...
sock.onmessage = function(e) {
	...
	var history = JSON.parse(e.data);
	
	// 填入数据
	for (var i = 0; i < history.dataSet.length; i++) {
		for (var j = 0; j < history.dataSet[i].length; j++) {
			chartDataSet[i].push(history.dataSet[i][j]);
		}
	}
	mainChart.setOption({
		dataset : {
